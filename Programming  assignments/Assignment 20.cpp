// Assignment 20.cpp : Defines the entry point for the console application.
//
#include "stdafx.h"
#include <stdio.h>
#include <string.h>
...
void findRecord(int id)
{
	...
	while (current != NULL)
	{
		if (current->ID == id)
		{
			printf("%d: %s %s: Math<%c> English<%c> French<%c> Philosophy<%c>\n",current->ID, current->firstname, current->surname, current->maths, current->english, current->french, current->philosophy);
			return;
		}
